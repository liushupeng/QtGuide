...

{
    ...
    TextEditor::setFont(
        QFont(configManager->getEditorFontFamily(), configManager->getEditorFontSize()));
...
bool TextEditor::maybeSave()
{
    ...
    switch (ret) {
    case QMessageBox::Save: save(); return true;
...
bool TextEditor::open()
{
    if (maybeSave()) {
        ...
        if (!file.open(QFile::ReadOnly | QFile::Text)) {
            QMessageBox::warning(
                this, tr("Warning"), tr("Cannot open file: ") + file.errorString());
            qWarning() << "[WARN 1] Failed to open" << fileName << ":" << file.errorString();
            return false;
        }
...
void TextEditor::openFile(const QString& fileName)
{
    ...
    if (!file.open(QIODevice::ReadOnly | QFile::Text)) {
        ...
        qWarning() << "[WARN 2] Failed to open" << fileName << ":" << file.errorString();
        return;
    }
...
void TextEditor::save()
{
    ...
    if (file.open(QFile::WriteOnly | QFile::Text)) {
        ...
        if (!file.commit()) errorMessage = "[WARN 5] Cannot save file: " + file.errorString();
    }
    else {
...
void TextEditor::saveAs()
{
    ...
    if (!file.open(QFile::WriteOnly | QFile::Text)) {
        QMessageBox::warning(this, tr("Warning"), tr("Cannot save file: ") + file.errorString());
        qWarning() << "[WARN 4] Failed to save" << fileName << ":" << file.errorString();
        return;
    }
    ...
    if (!file.commit()) {
        QMessageBox::warning(
            this, tr("Warning"), tr("Cannot save file: ") + fileName + file.errorString());
        qWarning() << "[WARN 6] Failed to save" << fileName << ":" << file.errorString();
    }
}
...
void TextEditor::contextMenuEvent(QContextMenuEvent* event)
{
    ...
    // Read only
    QAction* readOnlyAct = new QAction(QIcon(":/icons/read_only.svg"), tr("Read-Only mode"), this);
    ...
    for (const auto& def : m_repository.definitions()) {
        ...
        if (def.name() == m_highlighter->definition().name()) action->setChecked(true);
    }
...
void TextEditor::keyPressEvent(QKeyEvent* e)
{
    if (e->key() == Qt::Key_Tab) {
        if (this->textCursor().hasSelection()) {
            qDebug() << "wtf";
            addInEachLineOfSelection(QRegularExpression("^"),
        ...
        this->moveCursor(QTextCursor::MoveOperation::Left, QTextCursor::MoveAnchor);
        e->accept();
    }
    else if (e->key() == '{') {
        this->insertPlainText("{}");
...
void TextEditor::addInEachLineOfSelection(const QRegularExpression& regex, const QString& str)
{
...
void TextEditor::switchMode(const int& mode)
{
    if (mode == 0) {
        ...
        QPlainTextEdit::setFont(
            QFont(configManager->getEditorFontFamily(), configManager->getEditorFontSize()));
        setStyleSheet("background: " + QString::number(m_highlighter->theme().editorColor(
        ...
        setViewportMargins(0, 0, 0, 0);
        currentMode = mode;
        setFont(QFont(QFontDatabase::systemFont(QFontDatabase::GeneralFont).toString(),
                      configManager->getEditorFontSize()));
