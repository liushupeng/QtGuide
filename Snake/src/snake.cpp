...
bool Control::GoAhead(char userInputDirection)
{
    ...
    case 'a':
    case 'A':
        return GoAhead(0, -1);//Following the logic of moving up
    case 'd':
    case 'D':
        return GoAhead(0, +1);//Following the logic of moving up
    case 's':
    case 'S':
        return GoAhead(+1, 0);//Following the logic of moving up
    default:
        return true;
    }
}
...
bool Model::IsGameOver(int row, int col) const
{
    ...
    // The xy is the destination the snake head intends to go and this destination will cause gomeover
    // for example, index outof range
    // for example, the destination is snake body
    return (row < 0
            || row >= static_cast<int>(m_playBoard.size())
            || col < 0
...
void Model::ShowGame(QPainter &painter, int pictureSize) const
{
    ...
    for (size_t row = 0; row < m_playBoard.size(); row++)
    {
        for (size_t col = 0; col < m_playBoard[row].size(); col++)
        {
            ...
            if (row == static_cast<size_t>(headPosition.first) && col == static_cast<size_t>(headPosition.second))
            {
            ...
            painter.drawPixmap(col * pictureSize, row * pictureSize, pictureSize, pictureSize, pixmap);
        }
    }
}
